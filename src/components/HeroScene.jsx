...
function NeuralCore() {
  ...
  useFrame(({ clock, mouse }) => {
    ...
    group.current.rotation.y = clock.elapsedTime * 0.12 + mouse.x * 0.2;
    group.current.rotation.x = mouse.y * 0.14;
  });
    ...
    <group ref={group}>
      {points.map((point, i) => (
        <mesh key={i} position={point}>
          <sphereGeometry args={[0.04 + (i % 3) * 0.012, 8, 8]} />
      ...
      <lineSegments>
        ...
        <lineBasicMaterial color="#66e3ff" transparent opacity={0.28} />
      </lineSegments>
      <mesh>
